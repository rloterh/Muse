...
import { ParallaxText, HorizontalScroll, Counter, TextSplitReveal, ScrubScale } from "@/components/gsap/scroll-animations";
    ...
    <>
      ...
      {homepage.clientLogos && homepage.clientLogos.length > 0 && (
        <section className="px-8 py-14 lg:px-12">
          <div className="mx-auto max-w-7xl">
            <Reveal>
              <p className="text-xs font-medium uppercase tracking-[0.3em] text-[var(--color-text-dim)]">Selected partners</p>
            </Reveal>
            <div className="mt-8 grid grid-cols-2 gap-6 sm:grid-cols-3 lg:grid-cols-5">
              {homepage.clientLogos.map((logo, index) => (
                <div key={`${logo.alt ?? "logo"}-${index}`} className="flex h-24 items-center justify-center border border-[var(--color-border)] bg-[var(--color-bg-elevated)] px-6">
                  <SanityImage
                    image={logo}
        ...
        <div className="mt-16 pl-8 lg:pl-12">
          <HorizontalScroll>
            {featuredWork.map((project) => (
              <Link key={project.slug.current} href={`/work/${project.slug.current}`} className="group block w-[70vw] shrink-0 sm:w-[45vw] lg:w-[35vw]">
                <ScrubScale>
                  <div className="relative aspect-[4/3] overflow-hidden" style={{ backgroundColor: `${project.color ?? "#C8956C"}12` }}>
                    {project.coverImage?.asset ? (
                      ...
                      <div className="absolute inset-0 flex items-center justify-center">
                        <span className="font-display text-[12rem] font-bold leading-none opacity-[0.04]" style={{ color: project.color ?? "#C8956C" }}>
                          {project.title[0]}
                        </span>
                      </div>
                    )}
      ...
      {testimonials.length > 0 && (
        <section className="border-t border-[var(--color-border)] px-8 py-32 lg:px-12">
          <div className="mx-auto max-w-7xl">
            <Reveal>
              <p className="text-xs font-medium uppercase tracking-[0.3em] text-[var(--color-text-dim)]">Testimonials</p>
              <h2 className="mt-3 font-display text-4xl font-bold tracking-tight lg:text-5xl">What clients say</h2>
            </Reveal>
              ...
              {testimonials.map((testimonial, index) => (
                <Reveal key={`${testimonial.author}-${index}`} delay={index * 0.1}>
                  <div className="border border-[var(--color-border)] bg-[var(--color-bg-elevated)] p-8">
                    ...
                    <div className="mt-8">
                      <p className="text-sm font-medium text-[var(--color-text)]">{testimonial.author}</p>
